...
- [x] Phase 8 — docs, `/pricing` quoting the link's offer - 2026-08-27

**User request**: "a coupon code system in our admin portal that allows someone to sign up a new
store using a link generated by the discount, which gives them one year free… one time coupons…
also support multi use coupons… record in the database what coupon was used when a user signs up…
when the customer goes to enter billing information, they should be able to enter a coupon code…
a tab to show all of the coupons that have been redeemed."

### Decisions taken in the plan

**`platform_coupons`, not `coupons`.** The existing `coupons` table is a *merchant's* storefront
discount for shoppers — flow B, `store_id`-scoped, the merchant's money. A signup coupon is flow A:
platform-wide, RebelShops' money. Same word, unrelated features, so every identifier carries the
`platform` prefix and `/admin/coupons` is untouched.

**The console goes to `/platform`, not `/admin`.** Issuing a coupon that costs RebelShops money is
an operator action, and `/admin` is the merchant's portal. This makes `/platform` its first write
surface, which retires the reasoning behind best-effort `recordAdminAction` for mutating routes.

**"Months free" and "percent off" collapse into `percent_off` + `duration_months`.** A year free is
`100` / `12`; free forever is `100` / `NULL`. Percentage rather than `amount_off` because the intro
coupon's `amount_off` already forces `ensureIntroCoupon` to throw when the list price moves.

**One-time versus multi-use is `max_redemptions`,** not a `kind` column: `1`, `N`, or `NULL`.

**The coupon code box cannot be Stripe's.** `allow_promotion_codes` and `discounts` are mutually
exclusive in a Checkout Session — that pairing broke every subscription checkout on 2026-08-26 — so
the field is ours, validated by our API, passed as `discounts: [{ coupon }]`.

**Signup attributes; subscribing redeems.** A single-use code reserved at signup and consumed at
subscribe means an abandoned wizard does not permanently burn it. Reservations expire on cron and
release rather than delete, because "clicked 40 times, redeemed twice" is the number that says
...
collects nothing at signup. It maps to one Checkout Session parameter,
`payment_method_collection: 'if_required'`, so both kinds share one coupon shape rather than
...
the only thing that will ever ask that merchant for a payment method, and its button opens the
existing billing portal. There is no transactional email in this repo, so a merchant who does not
sign in during the last 30 days is never told — an accepted limit, written down in the plan's §12.

### Phase 8 — docs, and `/pricing` quoting the link (2026-08-27)

Read what phases 1-7 actually built rather than trusting the plan as a description of the shipped
code, per the phase-8 brief. It mostly held up; four places had drifted, and the docs now say what
the code does rather than what was originally proposed:

- **Economics immutability went stricter than planned.** The plan said "immutable once someone has
  redeemed"; `updatePlatformCoupon()` refuses `percentOff` / `durationMonths` / `collectPaymentMethod`
  in *every* patch, redeemed or not, because a Stripe coupon cannot be edited once it exists and any
  window where the row could drift from it ends in a wrong price on a real invoice. The plan (§11
  invariant 5) already said this before phase 8 — phase 8's job was to carry the same correction into
  `docs/payments.md` and `stripe/CLAUDE.md`, which still described the laxer rule.
- **The redemption close-out rides `checkout.session.completed`, not `customer.subscription.created`,**
  even though Stripe fires both for a new subscription — the handler that fires reuses an existing
  `expand: ['discounts']` retrieve and an existing `owner_id` resolution, so no second Stripe call and
  no second, possibly-disagreeing owner lookup. Not stated by name anywhere the reader would find it
  before phase 8; now called out in `docs/payments.md` §8 and `stripe/CLAUDE.md`'s new
  "Platform signup coupons" section.
- **Two window constants live in their own dependency-free module**, `billing/coupon-windows.ts`
  (commit `7fe833a`, landed *after* the plan's phase-7 text was written) — the plan's §5.1 still
  described the two clocks without naming where they'd end up living. Docs now point at that module,
  not at `coupon-claims.ts`/`discount-notice.ts` as if the numbers originated there.
- **The `/join` cookie's name/lifetime live in `_lib/coupon-cookie.ts`**, the same shape of fix as the
  window constants and already recorded above under "A cookie name should not import the JWT stack" —
  phase 8 only needed to make sure `stripe/CLAUDE.md` and `docs/payments.md` referenced the real module
  rather than paraphrasing.

**`/pricing` now honours the link (§14 decision 4, built).** `src/app/pricing/page.tsx` reads the
`rs_platform_coupon` cookie server-side via `next/headers`'s `cookies()` — the only way to read an
httpOnly cookie at all — re-validates the code with `isRedeemable()` rather than trusting `/join`'s
earlier verdict, and renders a coupon-specific page (`CouponPricingPage` / `CouponPlanCard`, new
files under `src/app/pricing/`) quoting `describePlatformCoupon()`'s real offer sentence and
`requiresPaymentMethod()`'s honest card answer in place of the standard "$1 for 3 months, then
$19.99". An invalid, expired, exhausted or deactivated code degrades silently to the standard page —
`/join` and the onboarding wizard already say what went wrong, so `/pricing` doesn't say it twice.
Calling `cookies()` opts the route into per-request dynamic rendering (confirmed in `next build`'s
output: `/pricing` moved from `○` static to `ƒ` dynamic), which is exactly what a per-visitor quote
needs and does not affect any other route.

No environment variable was added — checked the coupon commits' diffs and `.env.example` directly;
`CRON_SECRET` (already existed) is reused for `/api/cron/coupon-sweep`.

Verified: `tsc` clean, lint clean, **1377** unit tests (7 new, covering `CouponPricingPage`'s
card-requirement and pricing logic across 100%/partial/forever coupon shapes), and a keyless
production build registers `/pricing` as dynamic with no console errors during static-generation
fallback.
