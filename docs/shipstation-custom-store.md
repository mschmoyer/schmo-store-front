...
**Status in this repo:** we already have a Custom Store endpoint. See
[How this maps to our implementation](#how-this-maps-to-our-implementation) at the bottom for the
spec-to-code map.
...
- **Export excludes cancelled and refunded orders** (`AND o.status NOT IN ('cancelled',
  ...
  about the cancellation from us. Deliberate or not, it is a deviation — worth a look before
  relying on cancellation propagation.
...
Related: `docs/audits/shipstation-audit.md` for a review of the integration, and
`docs/payments.md` for the `orders` / `order_items` schema the export reads from.
