 ...
 * named {@link FieldError} — never a thrown exception, and never a value that reaches
 * `src/lib/platform/coupons.ts` unchecked. Two refusals matter enough to call out:
 *
 ...
 *   hitting that CHECK constraint would get a 500 with a raw Postgres message; this turns it into a
 *   400 with a field name instead.
 * - **A `PATCH` naming `percentOff`, `durationMonths` or `collectPaymentMethod` is deliberately
 *   *not* stripped.** Those fields thread through to `updatePlatformCoupon`, which returns the
 *   typed `economics_immutable` refusal the route turns into a 409. Validating them away here would
 *   silently swallow the attempt instead.
 */
 ...
 *
 * Generated codes (`coupon-codes.ts`) carry ~49.5 bits of entropy; a human-chosen code like
 * `FRIENDS12` does not. Without this floor, a short, guessable operator code plus a rate-limited
 * but unauthenticated `/join` made a year-long discount reachable by brute force at bandwidth
 * speed. Six characters over `COUPON_CODE_PATTERN`'s alphabet is `36^6` ≈ 2.2 billion combinations
 * — not generated-code entropy, but enough to make guessing through the rate limit impractical,
 * without rejecting the kind of code an operator actually wants to type.
 */
 ...
 * or invite a code that looks different than it is (leading/trailing space, a stray control char).
 */
...
export function validateCreateCouponBody(body: unknown): CreateCouponValidationResult {
  ...
  // See the module note: refusing this here, by field, keeps a typo from reaching
  // `platform_coupons_no_card_needs_full_discount` as a raw 500.
 ...
 * `percentOff`, `durationMonths` and `collectPaymentMethod` are validated for *type* only, then
 * passed through unchanged — see the module note.
 *
 * @param body - The parsed JSON body (or `null` when parsing failed upstream).
