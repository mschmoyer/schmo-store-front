...
const TestIntegrationSchema = z.object({
  integrationType: z.enum(['shipstation', 'stripe']),
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (integrationType === 'shipstation') {
      testResult = await testShipStationConnection(apiKey);
...
async function testShipStationConnection(apiKey: string) {
  try {
    // Log API key details for debugging (mask most of the key for security)
    const maskedKey = apiKey.length > 8 ? 
      apiKey.substring(0, 4) + '...' + apiKey.substring(apiKey.length - 4) : 
      '****';
    console.log(`Testing ShipStation with API key: ${maskedKey} (length: ${apiKey.length})`);
    
    // Validate API key format
    if (!apiKey || apiKey.length < 20) {
      return {
        success: false,
        error: 'Invalid API key format. ShipStation API keys should be longer.',
        data: {
    ...
    // Test ShipStation v2 API with warehouses endpoint
    console.log('Testing ShipStation API with /v2/warehouses endpoint...');
    const warehousesResponse = await fetch('https://api.shipstation.com/v2/warehouses', {
      method: 'GET',
      headers: {
        'api-key': apiKey,
    ...
    console.log(`ShipStation warehouses API response: ${warehousesResponse.status} ${warehousesResponse.statusText}`);
    
    if (warehousesResponse.ok) {
      const warehousesData = await warehousesResponse.json();
      console.log('ShipStation warehouses API success:', warehousesData);
      return {
        success: true,
        data: {
          ...
          endpointTested: 'https://api.shipstation.com/v2/warehouses',
          warehousesFound: warehousesData.warehouses?.length || 0,
          note: 'Connected successfully to ShipStation API v2'
        }
      };
    }
    ...
    try {
      ...
      console.log('ShipStation warehouses error response:', errorData);
      ...
      if (warehousesResponse.status === 401 && billingError) {
        return {
          success: true,
          data: {
            message: 'ShipStation API key is valid',
    ...
    let errorMessage = 'ShipStation API connection failed';
    let suggestion = 'Check ShipStation API documentation for required permissions.';
    
    if (warehousesResponse.status === 401) {
      errorMessage = 'Invalid API key or authentication failed.';
      suggestion = 'Verify your API key is correct and has not expired. Check ShipStation dashboard for key status.';
      ...
      errorMessage = 'ShipStation API endpoint not found. This may indicate an API version issue.';
      suggestion = 'Verify you are using the correct ShipStation API version and endpoint.';
    }
    ...
    return {
      success: false,
      error: 'Failed to connect to ShipStation API',
      data: {
